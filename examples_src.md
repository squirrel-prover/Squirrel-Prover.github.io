---
  title: Squirrel Prover
  pagetitle: Examples
---

The following examples are replays of interactive proofs done with the Squirrel Prover. **Beware: these files use outdated syntax; fixed versions can be
found in our code repository.**

# General examples

 * [Basic Hash](examples/basic-hash.html)
 * [Private Authentication](examples/private-authentication.html)
 * [Toy Counter](examples/toy-counter.html)
 * [YubiKey](examples/yubikey.html)
 * [Signed DDH (role P)](examples/signed-ddh-P.html) - [Signed DDH (role S)](examples/signed-ddh-S.html)
 * [LAK](examples/lak-tags.html)
 * [LAK - full](examples/lak-tags-full-wa.html)
 * [Running Example Secrecy](examples/running-ex-secrecy.html)
 * [Running Example Oracle](examples/running-ex-oracle.html)

# MOVEP

 * [Basic Hash](examples/basic-hash-two.html)
 * [Basic Hash - Authentication](examples/basic-hash-wa.html)
 * [Basic Hash - Incorrect](examples/basic-hash-fail.html)
